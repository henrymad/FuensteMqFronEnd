...
export class BuildingDetailPage implements OnInit {
  ...
  goToProfileEmployee(userName:string, profile:string):void{
    this.router.navigate([`manager/${this.state}/${this.nameBuilding}/${profile}/${userName}`]);
  }

  goToProfileManager(){
    this.router.navigate([`manager/${this.state}/${this.nameBuilding}/${this.manager.role}/${this.manager.userName}`]);
  }
